...
class Settings:
    ...
    OWNER = "cloudsmith"
    SEVERITIES = ['Critical', 'High'] # sevirities to filter for
...
def main():
    ...
    csQuery = CloudSmithQuery(Settings.API_KEY, Settings.OWNER, SEVERITIES)
    splunkExtractor = SplunkDataExtractor()
    # REPOS = ["actions", "examples", "testing-public"]
    REPOS = ["examples"]
    for repo in REPOS:
        # fetch all packages for this repo
        all_packages = csQuery.get_all_packages(repo)
        ...
        filtered_vulnerabilities = filter(csQuery.matches_severity, package_vulnerabilities)

        # extract the parts that are needed by Splunk
        splunk_data = [splunkExtractor.extract_splunk_fields(p) for p in filtered_vulnerabilities]
        printToJSON(splunk_data, "splunkdata.json")
